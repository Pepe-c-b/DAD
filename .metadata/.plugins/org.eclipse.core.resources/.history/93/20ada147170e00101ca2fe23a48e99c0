...
public class SensorValuesController extends AbstractController {
    ...
    // Umbral de temperatura para activar/desactivar el buzzer
        ...
        getVertx().eventBus().consumer(RestEntityMessage.SensorValue.getAddress(), message -> {
            ...
            switch (databaseMessage.getMethod()) {
                case CreateSensorValue:
                    ...
                    // Obtener la entidad sensor a partir del idSensor presente en SensorValue
                    launchDatabaseOperation(DatabaseEntity.Sensor,
                            new DatabaseMessage(DatabaseMessageType.SELECT, DatabaseEntity.Sensor,
                                    DatabaseMethod.GetSensor, sensorValue.getIdSensor()))
                            .future().onComplete(res -> {
                                if (res.succeeded()) {
                                    Sensor sensor = res.result().getResponseBodyAs(Sensor.class);
                                    if (sensor != null) {
                                        // Obtener la entidad dispositivo a partir del idDevice presente en Sensor
                                        launchDatabaseOperation(DatabaseEntity.Device,
                                                ...
                                                .future().onComplete(resDevice -> {
                                                    ...
                                                    if (resDevice.succeeded()) {
                                                        // Publicar el mensaje MQTT en el canal del dispositivo
                                                        mqttClientUtil.publishMqttMessage(device.getMqttChannel(),
                                                                gson.toJson(sensorValue), handler -> {
                                                                    System.out.println(handler.result());
                                                                });

                                                        // Obtener la entidad grupo a partir del idGroup presente en Device
                                                        launchDatabaseOperation(DatabaseEntity.Group,
                                                                ...
                                                                .future().onComplete(resGroup -> {
                                                                    ...
                                                                    if (resGroup.succeeded()) {
                                                                        // Publicar el mensaje MQTT en el canal del grupo
                                                                        if (sensorValue.getValue() > TEMPERATURE_THRESHOLD) {
                                                                            mqttClientUtil.publishMqttMessage(group.getMqttChannel(),
                                                                                    "1", handler -> {
                                                                                        System.out.println(handler.result());
                                                                                    });
                                                                        } else {
                                        ...
                                        // Actualizar la entidad dispositivo con la marca de tiempo actual donde se ha modificado el último sensor
                                        launchDatabaseOperation(DatabaseEntity.Device,
